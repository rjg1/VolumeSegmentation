...
def main():
    ...
    anchor_id_a = plane_a.anchor_point.id
    anchor_id_b = plane_b.anchor_point.id
    p0a = plane_a.anchor_point.position
    ...
    Va = np.stack([plane_a.plane_points[i].position - p0a for i, j in alignment_matches])  # Nx3
    Vb = np.stack([plane_b.plane_points[j].position - p0b for i, j in alignment_matches])  # Nx3
    ...
    for ppt in plane_b.plane_points.values():
        pt = ppt.position # extract position
    ...
    aligned_ids_b = list(plane_b.plane_points.keys())
