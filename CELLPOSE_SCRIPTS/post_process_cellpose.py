...
def main():
    ...
    # make_mask_list(masks, output_list)
    make_outline_list(masks, output_list)
    export_csv(output_list)
    plot_zstack(masks, img)

def make_mask_list(masks, output_list):
...
def make_outline_list(masks, output_list):
...
def plot_zstack(masks, img):
    # # plot image with outlines overlaid in red
    for z in range(masks.shape[0]):
        print(f"showing z-plane {z} of {masks.shape[0]}")
        outlines = utils.outlines_list(masks[z, :, :])
        print(outlines)
        plt.imshow(img[z, :, :])
        for o in outlines:
